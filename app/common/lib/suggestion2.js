...
const Immutable = require('immutable/dist/immutable.min.js')
const urlParse = muon.url.parse
const _ = require('underscore')

const ageDecayConstant = 50
...
const getSortForSuggestions = (userInputLower) => {
  ...
  return (s1, s2) => {
    if (s1.toJS || s2.toJS) {
      throw new Error('These sorting functions are not meant for Immutable data!')
    }
...
const sortingPriority = (count, currentTime, lastAccessedTime, ageDecayConstant) => {
  ...
  const ageInDays = (currentTime - (lastAccessedTime || 1)) / oneDay
...
const getMapListToElements = (urlLocationLower) => ({data, maxResults, type,
  ...
  filteredData = filteredData && filteredData.toJS ? filteredData : Immutable.fromJS(filteredData)
